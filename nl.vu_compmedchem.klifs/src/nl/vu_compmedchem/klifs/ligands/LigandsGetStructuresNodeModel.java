...
public class LigandsGetStructuresNodeModel extends KlifsNodeModel {
	...
	private final SettingsModelString m_inputColumnName = new SettingsModelString(CFGKEY_INPUTCOLUMNNAME, "Ligand ID");

    // the logger instance
    private static final NodeLogger logger = NodeLogger
            .getLogger(LigandsGetStructuresNodeModel.class);

    /**
     * Constructor for the node model.
     */
    protected LigandsGetStructuresNodeModel() {
        super(1, 1);
    }
    ...
    protected BufferedDataTable[] execute(final BufferedDataTable[] inData,
    	...
    	logger.info("Executing KLIFS Ligands Get Structures node - retrieving structure information from KLIFS server.");

    	// Check input data and execute query
    	List<Integer> ligandIDs = new ArrayList<Integer>();
    	int columnIndex = inData[0].getDataTableSpec().findColumnIndex(m_inputColumnName.getStringValue());
    	for (DataRow inrow : inData[0]) {
    		int ligandID = ((IntCell) inrow.getCell(columnIndex)).getIntValue();
    		ligandIDs.add(ligandID);
    	}
